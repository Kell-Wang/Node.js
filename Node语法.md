...
### 4.5 包与npm包管理工具
#### 4.5.1 Node.js中的包: 在Node.js中，可以通过包来对一组具有相互依赖关系觉得模块进行统一管理。通过包的使用，我们可以将某个独立的功能封装起来。-- 在Node.js中，一个包事实上就是一个目录，其中包含了用于对包进行描述的json格式的package.json文件。在一个包中，通常包含以下内容:
